/**
 * `/operacoes/planos/novo` — os parâmetros gerais do plano operacional.
 *
 ...
 * de `criar` em `/operacoes/gise/operacoes`, e pelo mesmo motivo.
 *
...
export const load: PageServerLoad = async ({ locals, platform }) => {
	if (!isAdminGeral(locals.usuario)) redirect(302, '/operacoes/gise');
...
export const actions: Actions = {
	criar: async (event) => {
		...
		redirect(303, `/operacoes/planos/${criado.id}`);
	}
};
