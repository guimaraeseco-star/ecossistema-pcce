...
- [ ] Trabalha em (E66): ficha de um servidor lotado numa delegacia COM subunidade (Aracati, Russas, Tauá ou o próprio DPI SUL) → o campo "Trabalha em" aparece com a sede marcada e o posto/núcleo na lista; escolher o posto e salvar → a ficha passa a mostrá-lo, a lotação NÃO muda; num servidor de delegacia sem subunidade o campo não aparece; POST direto com o posto de OUTRA delegacia → "tem de ser a própria unidade de lotação ou uma subunidade dela"
- [ ] Trabalha em (E66) — movimentação: mover para outra unidade um servidor que trabalhava num posto → o local volta para a sede do destino
- [ ] Efetivo por local de trabalho (E66): ficha da Unidade de Atendimento de Fortim → o quadro de efetivo conta os servidores que trabalham no posto e a ficha da Delegacia de Aracati NÃO os conta mais (a lotação deles continua Aracati, visível na ficha de cada um)
- [ ] Lista de servidores na ficha (E66): a ficha de qualquer unidade traz, abaixo do efetivo, a seção "Servidores" com nome (link para a ficha), matrícula, cargo, designação e situação de hoje; quem só trabalha ali aparece com "lotado(a) em <a delegacia de lotação>"; unidade sem ninguém mostra "Nenhum servidor trabalha nesta unidade"
- [ ] Busca por partes (E70): em `/servidores`, digitar duas partes do nome fora de ordem e sem acento ("fernandes maelson") acha o servidor; a matrícula continua achando; o mesmo em `/escalas` (título/cidade), na busca de unidades (o seletor de unidade aceita "juazeiro delegacia"), em `/auditoria` e em `/auditoria/logs`
- [ ] Valores é do Super Admin (E65): como Admin Geral, a home não traz mais o cartão 'Atualização de valores' e `/valores` redireciona; como Super Admin, a aba Valores continua no menu e a tela abre e grava normalmente
- [ ] Minha vida funcional (E53): entrar como servidor COMUM (sem papel) → `/perfil` mostra, abaixo do cadastro, a seção 'Minha vida funcional' com o cartão de férias e a linha do tempo (férias, afastamentos com CID, movimentações, designações, posse) e a seção 'Minhas escalas' com os dias escalados, mês a mês, os futuros abertos e os passados fechados, cada dia com link para a escala. **Nenhum botão de ação** em lugar nenhum: nada de corrigir, excluir, lançar programação, registrar abono ou dar ciência — a correção continua vindo do administrador da unidade
- [ ] Código na COTIC: Super Admin → `/unidades` → editar uma unidade → o campo `Código na COTIC` grava (inclusive com zero à esquerda) e aparece na ficha da unidade; repetir o mesmo código em outra unidade responde 409 dizendo que é o código; várias unidades sem código convivem sem erro
- [ ] Fila e avisos pelo nó (E65): com dois departamentos cadastrados, o Admin Geral de um NÃO vê em `/solicitacoes` os pedidos de servidores do outro, nem os avisos sobre eles em `/avisos`; POST direto em `?/decidir` ou `?/decidirAcao` com o id de um pedido de fora responde 403 e o pedido continua pendente. O que é do próprio nó continua aparecendo e decidindo normalmente. O badge de pendências conta só o que a fila mostra
- [ ] Nó do administrador (E65): entrar como Super Admin → menu "Administradores" → a lista mostra cada conta com o nó que ela administra e marca as que não operam; promover um servidor SEM escolher o nó é recusado; com o nó, a conta nasce administrando aquele departamento; tirar o nó de uma conta faz a tela dela dizer que não alcança unidade nenhuma. Como Admin Geral, `/administradores` responde 403, e na ficha do servidor o cartão de Admin Geral fica só de leitura
- [ ] Os dois chapéus (E71): entrando como Admin Geral do DPI SUL, o seletor "Departamento / Minha unidade" aparece no topo. Em **Departamento**: a Gestão de unidade mostra as seccionais e as delegacias, e `/ferias` traz o departamento inteiro. Em **Minha unidade**: a Gestão de unidade mostra só o DPI SUL e o núcleo de Juazeiro, `/ferias` traz só quem é lotado no departamento, e a lista de servidores encolhe junto. A caixa de avisos NÃO muda com o chapéu — é a exceção declarada
- [ ] Papel de administrador de unidade num DEPARTAMENTO (E71): entrar na sessão de SERVIDOR de quem tem papel de administrador da unidade 2 → a Gestão de unidade mostra só o DPI SUL e o núcleo de Juazeiro (nada de seccionais nem delegacias) e `/ferias` traz só quem é lotado no departamento. Um administrador de DELEGACIA continua vendo a delegacia e os postos dela, e o de SECCIONAL continua vendo as delegacias abaixo
- [ ] Seletor da SECCIONAL (E71): entrar como admin de seccional → o seletor no topo diz 'Seccional / Minha unidade'. Em **Seccional**: a Gestão de unidade traz as delegacias abaixo e `/ferias` traz a seccional inteira. Em **Minha unidade**: só a própria seccional — é desse chapéu que ela faz os pedidos da própria casa (férias, escala, afastamento dos servidores dela)
- [ ] Direção só do DPI SUL (E67): como admin de seccional ou de unidade, abrir a ficha de uma unidade → o cartão Direção mostra quem dirige, mas NÃO há botão de propor/registrar; POST direto em `?/proporDirecao` → 403 com "a direção da unidade é indicação do DPI SUL". Como Admin Geral, o botão "Registrar direção" continua lá, e agora o NUP é obrigatório (salvar sem NUP → recusa) e não há campo de portaria
- [ ] Respondência temporária (E68), Admin Geral: lançar férias para um delegado TITULAR começando em até 5 dias → a ficha da unidade passa a mostrar o aviso "…entra de férias em DD/MM e ninguém responde pela unidade" e a caixa de avisos traz a pendência; registrar a respondência (o adjunto da casa vem sugerido) → o aviso e a pendência somem, o titular continua titular (a linha dele não é encerrada) e, do primeiro ao último dia, o cartão Direção mostra quem responde com "Respondendo temporariamente até DD/MM"
- [ ] Respondência temporária (E68), unidade e seccional: com o mesmo cenário, entrar como admin da unidade → o botão é "Indicar quem responde" e a justificativa é obrigatória; enviar → o pedido aparece em `/solicitacoes` para o DPI SUL; aprovar → a cobertura passa a valer; rejeitar → nada é registrado
- [ ] Respondência temporária (E68), fim antecipado: registrar retorno antecipado do titular afastado → a cobertura encurta no mesmo dia; corrigir a fração de férias para terminar antes → idem; excluir a programação de férias → a cobertura fecha e larga o vínculo com o evento
- [ ] Aviso de respondência (E68) → lista: com duas ou mais unidades sem quem responda, o botão 'Resolver' da caixa de avisos abre `/unidade?pendencia=respondencia` já filtrado, mostrando só essas unidades; com uma só, abre a ficha dela. Na lista de unidades, cada uma traz a linha '⚠ FULANO entra de férias em DD/MM — sem quem responda' e há o botão 'Sem respondência · N' para ligar e desligar o filtro
- [ ] Papel × função (E69): na ficha do servidor, o campo Designação diz "função na estrutura" e o cartão Papel Administrativo diz que é acesso ao sistema, que pode ser de delegado ou inspetor e que quem dirige a unidade é o Delegado Titular
- [ ] Subunidade (E66): Super Admin → `/unidades` → "Cadastrar" → o seletor de tipo agora tem "Subunidade" → escolher o que é (Posto/Unidade, Núcleo, Seção, Célula) e a unidade a que pertence (uma delegacia ou o departamento) → a subunidade aparece na árvore sob a unidade escolhida e na ficha dela
- [ ] Designar como titular (E66): cadastrar um servidor DPC numa unidade SEM titular → aparece o bloco "Designar como titular", com data e NUP → salvar → a ficha da unidade mostra a direção; repetir numa unidade que JÁ tem titular → o bloco não aparece; em OIP também não. Movimentação de um DPC para unidade sem titular → a mesma caixa, usando a data e o NUP da movimentação
- [ ] Direção só em delegado (E69): na ficha de um OIP, escolher a designação "Delegado Titular" (ou Adjunto/Auxiliar) e salvar → recusa "só cabe em delegado (DPC)"; "Chefe de seção…" no mesmo OIP → aceita
- [ ] Relatório diário dos colaboradores (E61-b) — `[Vitest: relatorio-diario.test.ts]` cobre agrupamento, hora de Brasília, destinatário sem e-mail e idempotência. Manual: garanta que o admin da unidade tem e-mail pessoal (Meu perfil); faça uma ação como colaborador; dispare o workflow "Relatório diário de colaboradores" na aba Actions (`workflow_dispatch`, campo `dia` vazio = hoje) → o admin recebe "Colaboradores — Unidade — dd/mm/aaaa" com uma tabela por colaborador (hora, ação, sobre quem); disparar de novo → resposta com `jaEnviados` e nenhum e-mail; unidade cujo admin não tem e-mail pessoal aparece em `semDestinatario`; dia sem ação → `unidadesComAcao: 0`; localmente: `POST /api/webhook/relatorio-colaboradores` com `Authorization: Bearer $SYNC_TOKEN`, `X-Webhook-Timestamp` e `X-Webhook-Nonce`
- [ ] Teto de férias (E64): Início → Gestão de pessoal → cartão "Férias" (e item "Férias" na barra) → abre o panorama da SUA unidade (seccional: a seccional; Admin Geral: o departamento); em cada mês, duas linhas por unidade — "teto: N de E · X %" (só quem tem o 1º período COMEÇANDO no mês; ⚠ acima de 15 %) e "em férias: M de E · Y %" (qualquer fração); uma 2ª fração no mês entra só na segunda linha; ao lançar uma 1ª fração, o aviso do teto diz "Teto: N de E com o 1º período iniciando no mês (X %)" e, à parte, o "em férias"; ao lançar a 2ª/3ª não há aviso de teto
- [ ] Área do colaborador (E64): os cartões são os mesmos do Início (azuis), um por destino — Servidores (com "Você pode: …"), Férias (só com a chave de férias → `/ferias`), Escalas ordinárias, Avisos (com o badge)
- [ ] Colaborador na unidade (E61) — lotação: Admin Geral → `/colaboradores` → o cadastro tem "Unidade em que está lotado(a)"; na lista, a coluna "Unidade" é um select que salva ao mudar e o link "acessos" abre a ficha da unidade
- [ ] Colaborador na unidade (E61) — sem acessos: entrar como colaborador lotado sem nada marcado → `/colaborador` diz que a unidade ainda não liberou nada; `/servidores`, `/avisos`, `/unidade/[id]` e `/escalas` voltam para `/colaborador`; a barra só tem "Boas-vindas"
- [ ] Colaborador na unidade (E61) — a unidade define: como admin da unidade → `/unidade/[id]` → bloco "Colaboradores" → "Definir acessos" → marcar "Propor afastamento", "Ver as escalas", "Ler os avisos" → "Salvar acessos" → o resumo mostra também "Ver os servidores da unidade" (implícita); como admin de SECCIONAL fazendo o mesmo → a unidade recebe a notícia "Acessos do colaborador … alterados" no cartão Unidade; a auditoria tem `definir_acessos_colaborador` com antes/depois
- [ ] Colaborador na unidade (E61) — com acessos (entre de novo, ou espere 1 min — cache de sessão): a barra ganha Avisos, Servidores, Minha unidade, Escalas; `/servidores` lista só a unidade dele; a ficha abre em modo solicitação com o painel de afastamento, SEM o botão "Solicitar alteração" (aviso "A unidade não liberou…") e com o cartão de férias sem botões; propor um afastamento funciona e o pedido aparece na fila do DPI SUL como "Fulano (colaborador — Unidade)"; ficha de servidor de OUTRA unidade → 403; `/colaborador/escalas` lista o mês com link para a escala, que abre sem botões de edição; `/escalas` (lista dos admins) continua fora; `/avisos` mostra a caixa da unidade
- [ ] Colaborador na unidade (E61) — retirar: desmarcar tudo → no próximo minuto o colaborador volta à tela vazia; deslocar (select "— sem lotação —" em `/colaboradores`) → idem, mesmo com chaves gravadas
- [ ] Abono, dois limites (E62): servidor com abono deferido num exercício → nas outras frações do MESMO exercício "Registrar abono" some e no lugar aparece "sem abono — o exercício X já teve abono"; num exercício diferente cujo período cairia no MESMO ano civil → "sem abono — já há abono em AAAA"; POST direto na action → 409 com a mesma razão; registrar um INDEFERIMENTO continua permitido
- [ ] Fração com abono intocável (E63): fração com abono deferido mostra "· não reprogramável (art. 4º § 1º)"; "Reprogramar" só aparece se houver outra fração sem abono por gozar; no modal a fração com abono aparece em cinza como "fica como está" e não entra na soma nem na redivisão (30 com 10 vendidos → sem botão; 15+15 com 10 vendidos da 1ª → só a 2ª, redivisão 15; 10+10+10 com uma vendida → as outras duas, 20 ou 10+10); fração em gozo COM abono não pede retorno
