...
- [ ] Retorno antecipado: no quadro "Solicitações deste servidor", o pedido de afastamento APROVADO e em curso (não férias) mostra o botão "Retorno antecipado", que abre data do retorno (limitada ao período) e NUP. Como admin de unidade, vira um PEDIDO "Retorno antecipado" pendente (card com Afastamento, Início, Término previsto, Retorno ao serviço e NUP); o botão some enquanto o pedido está pendente ou aprovado e volta se rejeitado; na fila `/solicitacoes`, "Aprovar retorno antecipado de …" encurta o afastamento até a véspera (descrição "Retorno antecipado em … (NUP …)") e a unidade recebe a notícia; rejeitar não muda nada. Como Admin Geral, registra direto. Retorno fora do período → 409; a linha do tempo NÃO tem esse botão (só Corrigir/Excluir do Admin Geral)
